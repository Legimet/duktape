...
#if 0
DUK_EXTERNAL duk_bool_t duk_XXX_boolean(duk_context *ctx, duk_idx_t idx, duk_bool_t def_value) {
	DUK_ASSERT_CTX_VALID(ctx);
...
DUK_LOCAL DUK_ALWAYS_INLINE duk_double_t duk__getopt_number(duk_context *ctx, duk_idx_t idx, duk_double_t def_value) {
	...
	if (DUK_TVAL_IS_DOUBLE(tv)) {
		...
		ret.d = def_value;
#if 0
		/* Would be necessary for user defaults. */
...
#if 0
DUK_EXTERNAL duk_double_t duk_XXX_number(duk_context *ctx, duk_idx_t idx, duk_double_t def_value) {
...
#if 0
DUK_EXTERNAL duk_int_t duk_XXX_int(duk_context *ctx, duk_idx_t idx, duk_int_t def_value) {
	DUK_ASSERT_CTX_VALID(ctx);

	return (duk_int_t) duk__api_coerce_d2i(ctx, idx, def_value, 0 /*require*/);
}

DUK_EXTERNAL duk_uint_t duk_XXX_uint(duk_context *ctx, duk_idx_t idx, duk_uint_t def_value) {
	DUK_ASSERT_CTX_VALID(ctx);
...
#if 0
DUK_EXTERNAL const char *duk_XXX_lstring(duk_context *ctx, duk_idx_t idx, duk_size_t *out_len, const char *def_ptr, duk_size_t def_len) {
	duk_hstring *h;
	const char *ret;
	duk_size_t len;
...
DUK_EXTERNAL const char *duk_XXX_string(duk_context *ctx, duk_idx_t idx, const char *def_ptr) {
	duk_hstring *h;
	...
	if (h != NULL) {
		...
		return def_ptr;  /* XXX: shared helper */
	}
}
#endif
...
#if 0
DUK_EXTERNAL void *duk_XXX_pointer(duk_context *ctx, duk_idx_t idx, void *def_value) {
...
#if 0
DUK_EXTERNAL void *duk_XXX_buffer(duk_context *ctx, duk_idx_t idx, duk_size_t *out_size, void *def_ptr, duk_size_t def_size) {
	DUK_ASSERT_CTX_VALID(ctx);

	return duk__get_buffer_helper(ctx, idx, out_size, def_ptr, def_size, 0 /*throw_flag*/);
}
#endif
...
#if 0
DUK_EXTERNAL void *duk_XXX_buffer_data(duk_context *ctx, duk_idx_t idx, duk_size_t *out_size, void *def_ptr, duk_size_t def_size) {
...
#if 0
DUK_EXTERNAL duk_c_function duk_XXX_c_function(duk_context *ctx, duk_idx_t idx, duk_c_function def_value) {
	duk_c_function ret;
...
#if 0
DUK_EXTERNAL duk_context *duk_XXX_context(duk_context *ctx, duk_idx_t idx, duk_context *def_value) {
	duk_context *ret;
